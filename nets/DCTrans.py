...
class Deform(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        offset_channel = config.transformer.num_heads
        self.offset_range_factor = config.offset_range_factor
        self.offset_conv = nn.Sequential(
            nn.Conv2d(offset_channel, offset_channel, config.offset_conv_k, 1, config.offset_conv_k//2),
            LayerNormProxy(offset_channel),
            nn.GELU(),
            nn.Conv2d(offset_channel, 2, 1, 1, 0, bias=False)
        )

    @torch.no_grad()
    def _get_ref_points(self, Ho, Wo, dtype, device):
        
        ref_y, ref_x = torch.meshgrid(
            torch.linspace(0.5, Ho - 0.5, Ho, dtype=dtype, device=device), 
            torch.linspace(0.5, Wo - 0.5, Wo, dtype=dtype, device=device)
        )
        ref = torch.stack((ref_y, ref_x), -1)
        ref[..., 1].div_(Wo).mul_(2).sub_(1)
        ref[..., 0].div_(Ho).mul_(2).sub_(1)
        
        return ref

    def forward(self, mhQ):
        dtype, device = mhQ.dtype, mhQ.device
        
        offset = self.offset_conv(mhQ)
        Ho, Wo = offset.size(2), offset.size(3)
        
        if self.offset_range_factor > 0:
            offset_range = torch.tensor([1.0 / Ho, 1.0 / Wo], device=device).reshape(1, 2, 1, 1)
            offset = offset.tanh().mul(offset_range).mul(self.offset_range_factor)
        
        offset = einops.rearrange(offset, 'b c h w -> b h w c')
        reference = self._get_ref_points(Ho, Wo, dtype, device)
        ...
        return Q_sampled, Q_sampled

########################################################################################

class DAttention(nn.Module):
    def __init__(self, config, vis, batch_size, channel_num):
        super(DAttention, self).__init__()
        self.vis = vis
        self.KV_size = config.KV_size
        self.channel_num = channel_num
        self.num_attention_heads = config.transformer["num_heads"]
        self.deform = Deform(config)
    ...
    def forward(self, emb1,emb2,emb3,emb4,emb5, emb_all):
        ...
        multi_head_Q = multi_head_K
        multi_head_K, multi_head_V = self.deform(multi_head_Q)
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, batch_size, channel_num):
        ...
        self.channel_attn = DAttention(config, vis, batch_size, channel_num)
...
class Encoder(nn.Module):
    def __init__(self, config, vis, batch_size, channel_num):
        ...
        for _ in range(config.transformer["num_layers"]):
            layer = Block_ViT(config, vis, batch_size, channel_num)
...
class DCT(nn.Module):
    def __init__(self, config, vis, img_size, batch_size, channel_num=[64, 128, 256, 512, 512], patchSize=[16, 8, 4, 2, 1]):
        super().__init__()
        ...
        self.encoder = Encoder(config, vis, batch_size, channel_num)
